...
| `data/company-checkups.tsv` | Your append-only company-checkup ledger (ADR-0025): `{tracker#}\t{date}\t{company-slug}\t{company}\t{star}\t{risks}\t{html}\t{note}`. `tracker#` is the join key to the tracker; `?` means no tracker row yet (company-name-mode checkup where invite-match found none — same `?` sentinel as the unknown-employer policy). `star` is the checkup skill's 1–5 employer-reliability rating (0.5 steps) — display/analytics only, never a scoring input. `risks` is a comma-joined closed set (`social-zero`, `social-mismatch`, `scale-mismatch`, `entity-confusion`, `arbitration`, `review-negative`, `tactics`, `media-negative`) or `-`. `html` points into `reports/checkups/` or `-`. Appended only by `lib/log-checkup.mjs` (which owns validation and the header line); never edited in place. The sole machine channel for checkup data — reports carry no Machine Summary key. Read by the dashboard and `analyze-patterns.mjs` |
