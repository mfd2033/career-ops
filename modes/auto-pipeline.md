...
**Chinese job boards (猎聘 `liepin.com` / BOSS直聘 `zhipin.com`·`kanzhun.com` / 智联招聘 `zhaopin.com`):** these wall off headless *and* logged-out browsers with a captcha/anti-bot challenge, so `browser_navigate`, `WebFetch`, and `WebSearch` all return empty on them. For any URL on those hosts, **skip the steps below** and run `node browser-extract.mjs <url>` (default `--extractor auto` auto-routes zh boards to the candidate's own logged-in browser via `bsk`, handing a slider captcha to the candidate through `request-help`). Use its `text` as the JD. If it exits non-zero with a `bsk_*` code (e.g. `bsk_missing`), **stop and tell the candidate to install browser-skill and run `bsk status`** — never infer the JD from similar postings. The `bsk` CLI + browser-skill extension must be connected for this to work.

1. **Playwright (preferred):** Most job portals (Lever, Ashby, Greenhouse, Workday) are SPAs. Use `browser_navigate` + `browser_snapshot` to render and read the JD.
   - **Opt-in — CLI extractor (`scan.extractor: cli` in `config/profile.yml`):** run `node browser-extract.mjs <url>` (default `--mode jd`) instead; it returns compact `{ "url", "title", "text" }` — just the distilled JD main text rather than the full page a11y tree, so the model processes fewer tokens (board-dependent — modest on clean boards, larger on chrome-heavy SPAs). For zh boards this path is **required** (not optional) — it is the only one that routes through `bsk`. Use its `text` as the JD. **Fall back silently** to `browser_navigate` + `browser_snapshot` if it errors or is missing (non-zh URLs only). Read-only (navigate + read, no clicks/fills), so it never applies to the apply-form step below.
