...
- **Status:** Accepted (2026-09-15) — **决议 4（同日去重）由 ADR-0033 取代（2026-09-17）**：同日/跨日一律不再按 agent-inbox 审计行拦截，改由「本行是否有体检在跑」判定；其余决议继续有效。
...
4. **去重语义随之更新。** 同 tracker# 当天已派发（dispatched 行）或已在队列的请求 → 409；台账 append-only 不变。**（本条已被 ADR-0033 取代：同日去重废止，改由「本行是否有体检在跑」判定，在跑时由用户裁决停止或替换。）**
