...
- **批量跳过（SKIP）与报告页单个跳过（Discarded）是两个不同落点**——都叫「跳过」，语义不同（动作 vs 状态，CONTEXT.md 词条）。本次只改批量路径，未动 `SkipFromTracker`；若日后要统一，需另改该组件（不在本 ADR 作用范围）。**（2026-09-18 修订：ADR-0040 已统一——单个跳过（报告页 + 今日页）也写 `SKIP`，本条区分不再成立。）**
