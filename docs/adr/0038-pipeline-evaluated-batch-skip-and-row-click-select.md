...
- **Context:** 需求（2026-09-18）：①在「已评估」页增加批量跳过功能；②列表项点击复选框之外的空白区域也能选中。现状与约束：
  ...
  2. **CONTEXT.md「跳过（skip）」词条已区分两种「跳过」**：报告页「跳过」动作 = 标记已放弃（**Discarded**），是动作不是状态；tracker 终态 **SKIP**（"不在考虑范围/不投递"，独立 tab）是另一概念，两者不可互推。现有单个跳过按钮（`SkipFromTracker`）写的正是 Discarded，且**无确认弹窗**。
...
1. **批量跳过写 `Discarded`（已放弃）**，与报告页单个跳过动作同一终态（CONTEXT.md「跳过」词条已定）。不写 `SKIP` 分组——保持与现有单按钮语义统一：行仍在 tracker、移出活跃队列、可经筛选找回。
