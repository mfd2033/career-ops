...
## Amendment (2026-09-26): 数据行光标 = 默认箭头，手型收窄为「仅跳转入口」

- **背景:** 去链后一行里出现两种点击语义——整行空白 = 勾选（决议 2/3）、公司名 = 跳转（决议 1）。但 `<tr>` 仍挂着 `cursor-pointer`，手型盖在「勾选」动作上，与「点行不跳页」相悖。需求（一轮 grilling 敲定）只问一件事：鼠标移到已评估数据行该显示手型还是默认箭头。
- **规范:** 手型光标只代表链接/导航。据此，**整行空白区用默认箭头**（点行仍切换勾选，不改 `onClick`）；行内原生 checkbox 与表头排序列、公司名真链接各自保留手型（原生控件 / 真链接不在此规则约束内）。可勾选线索由决议 6 既有的 `hover:bg` 底色承担，不靠光标；不加 `title`、不给公司名加下划线（沿用决议 6「热区与视觉不加补偿」）。
