...
**`{company-slug}` is a filename component, not a name.** Both the report file (`reports/{{REPORT_NUM}}-{company-slug}-{{DATE}}.md`) and the TSV filename must contain ONLY `[a-z0-9-]`. Never interpolate the raw company field into a path: this repo's own placeholder for an undisclosed employer is `?`, and `?`, `<`, `>`, `:`, `"`, `/`, `\`, `|` are illegal in Windows filenames — the MSYS/Git-Bash layer silently rewrites them into Private Use Area codepoints (`?` → U+F03F), so the write *succeeds* and leaves a file that no tool, link, or `git` operation can address. When the employer is `?` or the slug would otherwise be empty, do NOT interpolate it — resolve a filename-safe placeholder slug per the account's anonymous-employer slug policy (this repo's user-layer rule lives in `modes/_custom.md`, injected into this prompt at run time). The link you write in column 8 must name the report file exactly as you wrote it to disk — a slug that differs between the two is a dead link the tracker cannot detect or repair.
