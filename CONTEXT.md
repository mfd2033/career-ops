...
**运行引擎（run engine）**:
某个工作器在派发那一刻请求的 CLI 运行时与模型（运行时显示名 + 模型 id），是该次派发的**请求值快照**——不随配置页后续更改而变，也不代表执行中实际加载的模型（不做推断，不读实际值）。工作器可见处（/jobs 列表与卡片、侧栏、/jobs/{id} 详情页）如实展示；缺失时列表不占位、详情页明写「未记录（早于该功能）」。与**引擎模式**（决定走哪条通路）、**当前使用**（配置页此刻的存值）都不同：那两者说现在，它说当时。批量入口与 CLI 交互式路径不记录它（已知留白，ADR-0043）。
_Avoid_: AI 引擎（旧 UI 标题）、当前引擎（易误读为实时值）、引擎（与引擎模式混淆）

**厂商安装目录（vendor bin dir）**:
某个 CLI 运行时自己的安装目录（如 Qoder CN 的 `%USERPROFILE%\.qodersec\bin`），写在引擎规格的 `binDirs` 里。只在 PATH 与通用搜目录都查不到时才生效：PATH 永远优先，它只是兜底。
_Avoid_: 额外搜索路径、自定义 PATH

**动态模型清单（dynamic model list）**:
运行时的可选模型不静态声明，而是运行时问 CLI 自己（opencode 的 `opencode models`、Qoder CN 的 `qoderclicn --list-models`）。问不到就是空清单，不拿静态副本兜底。
_Avoid_: 模型列表（那是控件名）、内置模型

**用量申报（usage claim）**:
CLI 在 run 结束时自报的 token 数与费用，界面据此显示「这次花了多少」。有的运行时不申报（Qoder CN 以 Credits 计费，`result.usage` 全为 0），此时如实不显示——显示 0 等于宣称这次没花钱。
_Avoid_: 成本、消耗、计费（Credits 是计费单位，不是这里说的申报动作）

**技能副本（skill copy）**:
同一技能在某个 agent 技能目录里的一份实体（目录含 SKILL.md）。同一技能可有多个副本且版本不同（中央库、各 CLI 目录、旧克隆并存）；版本读自 SKILL.md frontmatter，缺失时如实显示「未标注」。体检派发取最高版本副本解析出绝对路径注入 prompt（ADR-0056）。
_Avoid_: 技能安装（一个副本才谈得上装）、技能目录（那是副本的容器）

**技能面板（skills panel）**:
配置页上展示本机 agent 技能安装情况的区块：每个聚焦技能一张卡片，聚合各技能副本的安装路径与版本号，未安装以徽标如实呈现。它是知情展示，不做安装/升级动作，也不拦截体检派发（ADR-0056）。
_Avoid_: 技能管理器（那是 skills-manager）、插件市场

**聚焦技能（featured skill）**:
技能面板 v1 只渲染的白名单技能（offer体检、browser-skill）。目录扫描器是通用的，白名单只控制展示面；加技能改白名单，不改扫描器。
_Avoid_: 支持技能（没说清只是展示）、受管技能（与 skills-manager 的管理语义混淆）
...
**报告薪资（report salary）**:
tracker 行显示与排序所用的薪资，事实源是该行评估报告 Machine Summary 的 `advertised_comp`——招聘方挂出来的薪资。展示为归一化月薪 K 区间（`10-15K`，字段原文悬停可见），解析不出即显示「—」并恒沉底；折算一律月薪口径（裸「万」= 月薪，只有显式「/年」「年薪」才 ÷12），非人民币不解析。
_Avoid_: 广告薪资、JD 薪资（与「收件箱薪资」的采集卡片口径混作一谈）
...
**批量条（batch bar）**:
列表上方承载「对当前勾选集施加动作」的那一条，管道追踪器（重新评估 / 已评估 tab 的批量跳过）与收件箱（存入候选清单 / 跳过选中）各有一条；作用是当前筛选后的列表，不是整个结果集。自 ADR-0039 起它住在**常驻槽位**里：桌面端（`md:`，与定高布局同界）槽位恒占一行且高度锁定，条的出现/消失与条内文案变化都不再挪动列表；窄屏没有槽位，条仍在出现时把列表下推（已知残差）。
_Avoid_: 批量操作栏（旧称）、操作栏、批量工具栏
...
**跳过（skip）**:
把岗位标记为终态 `SKIP`（"岗位不适合、不投递"）的动作，行归入管道「跳过」tab。所有叫「跳过」的入口落点一致（ADR-0040）：报告页单个跳过按钮、今日页「待决策」卡的跳过、管道已评估页的批量跳过（ADR-0038）都写 `SKIP`。「已放弃（Discarded）」是另一个终态（"候选人自己放弃 / offer 关闭"），不再由跳过按钮产生，只能经状态下拉框手写或来自历史数据。
_Avoid_: 略过、忽略、跳过=已放弃（ADR-0040 前的旧语义）
...
**执行起点（started running）**:
任务取得执行槽、CLI 被 spawn 的那一刻，与工作器卡片的派发时刻（含排队段，`Job.startedAt`）和入队时刻（池的 `enqueuedAt`）互不混用。工作器展示面的「始于 HH:MM」即执行起点（无排队段时回退派发时刻）；排队态显示的是入队时刻（「排队中 · 自 HH:MM」）。详情页的诚实时间链 = 排队于 → 始于 → 止于 · 时长，缺段省略、不回填不猜测（ADR-0044）。
_Avoid_: 开始时间（不区分排队段/执行段时的歧义源）、起跑时间

**批量逐项快照（batch item snapshot）**:
批量 run 终结（done/error/cancel 都算）时随台账记录落盘的逐项结论数组（label/ok/skipped/score/star/reason），单条上限 100 项、溢出仅记 total（ADR-0045）。区别于内存登记表的实时逐项数据（进程内、重启即清、run 结束后成为淘汰候选）：快照是历史回看的真相源，登记表只服务运行中/刚结束的跨页签恢复。展示口径两处同一共享组件：`/jobs` 列表展开与 ledger-only 详情视图。
_Avoid_: 把内存登记表当持久化数据源引用；「批量结果」（不区分快照/实时数据时的歧义源）

**批量子任务详情行（batch subtask detail row）**:
批量里每个**成功**子项在自己跑完的瞬间各写的一条独立 run 台账行（ADR-0046），用于把它当作一个工作器在 `/jobs/{id}` 打开。带 `parentId`（= 父批量 `serverBatchId`）关联父行；id 为确定性散列 `fnv1a(batchId::key)`，服务端落盘与前端卡片跳转两侧算出同一个值。列表只显示父批量行、子行经父卡片展开进入（`parentId` 存在即被 `/jobs` 列表排除）。失败/跳过/取消项不写子行。子行展示对齐「重开的历史单次任务」的诚实子集（状态/引擎/时间链/时长，无逐工具步骤流），报告目标 evaluate=`/report/{num}`、checkup=`/api/checkup-report?n={tracker#}`。区别于批量逐项快照（一行内嵌数组、含失败/跳过、只为计数与不可点清单）：子行是单个成功项的可点开真相源。
_Avoid_: 把子行当列表里的一级行；把快照 items 当可点详情入口（快照行不可点，可点靠子行 id）

## 启动器可观测性

**日志窗口（log window）**:
launcher 启动时弹并常驻、**属于 launcher 自己**的窗口（非 conhost 托管的控制台，因为跨进程无法拦截其关闭），在一条统一时间线上实时显示 launcher 决策与 dashboard 服务的输出。点 × 不等于退出（见「关窗即隐藏」），托盘「显示日志窗口」是唤回入口。启动失败时 launcher 不退出而是托盘驻留，失败现场永远可查。
_Avoid_: 控制台窗口（conhost 托管、不可拦截关闭，是被隐藏而非被唤回的对象）、调试窗口、tail 窗口

**统一启动器日志（launcher log）**:
日志窗口的磁盘孪生（`.career-ops-web/launcher.log`），逐行同内容、同「时间戳+来源前缀」格式。生命周期跟 launcher 进程走：进程启动时重置，托盘「重启服务」只追加分隔行不删崩溃前现场。它是无窗口场景（服务复用路径、脚本静默拉起）下唯一的运行记录。
_Avoid_: tray-debug.log（旧通道，只记 launcher 就绪后的日志且路径随版本目录漂移，被本词条取代）、服务日志（那指工作器 run 台账）

**关窗即隐藏（close-hides）**:
日志窗口对 × 的语义承诺（靠自持窗口拦截 WM_CLOSE 实现）：窗口消失但 launcher 与服务的生死不受影响，退出仍然只有托盘「退出」一条路。用户永远不需要为了「别把服务弄死」而不敢关窗。
_Avoid_: 最小化（不是缩到任务栏）、关窗退出（旧行为：连带杀死 launcher 留孤儿服务，正是本承诺消灭的对象）
