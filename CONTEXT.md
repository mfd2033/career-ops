...
**跳过（skip）**:
报告页上的动作：把这个岗位标记为「已放弃」，接着看下一份报告；没有下一份时回到列表。它不是 tracker 的终态之一——那个终态的含义是"岗位不适合、不投递"，中文显示恰好也叫「跳过」；两者一个是动作、一个是状态，不可互推。**批量跳过（管道已评估 tab，ADR-0038）**：勾选一批行后一次性统一写 `SKIP`（跳过分组，终态），仅出现在已评估筛选下。注意与报告页单个跳过写 `Discarded` 不同——两者都叫「跳过」但落点不同（动作 vs 状态）。
_Avoid_: 略过、忽略
