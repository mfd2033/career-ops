...
export function DeleteFromTracker({ n }: { n: string }) {
  ...
  async function openConfirm() {
    ...
    try {
      ...
      if (!r.ok) {
        setErr(d.error || t("pipeline.removeErrCant"));
        return;
      }
      setOrphan(d.orphanReport ?? null);
    } catch {
      setErr(t("pipeline.removeErrReach"));
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const d = await r.json().catch(() => ({}));
      if (!r.ok) {
        setErr(d.error || t("pipeline.removeErrFailed"));
        setBusy(false);
        return;
      }
      ...
      setErr(t("pipeline.removeErrFailed"));
      setBusy(false);
    }
  }
  ...
  if (!open) {
    return (
      ...
      >
        <Trash2 className="size-3.5" /> {t("pipeline.removeFromTracker")}
      </button>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-red-400/30 bg-red-500/[0.06] p-3 text-xs">
      <p className="font-medium text-foreground">{t("pipeline.removeConfirm", { n })}</p>
      <p className="mt-1 text-muted">
        {t("pipeline.removeWarn")}{orphan ? t("pipeline.removeOrphan", { file: orphan }) : ""}
      </p>
      ...
      <div className="mt-2.5 flex gap-2">
        ...
        >
          {busy ? <Loader2 className="size-3.5 animate-spin" /> : <Trash2 className="size-3.5" />} {t("pipeline.delete")}
        </button>
        <button
          disabled={busy}
