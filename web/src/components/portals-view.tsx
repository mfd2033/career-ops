...
const TONE: Record<string, { dot: string; labelKey: string; chip: string }> = {
  live: { dot: "bg-emerald-500", labelKey: "portals.statusLive", chip: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400" },
  empty: { dot: "bg-amber-500", labelKey: "portals.statusLiveEmpty", chip: "bg-amber-500/15 text-amber-700 dark:text-amber-400" },
  broken: { dot: "bg-red-500", labelKey: "portals.statusBroken", chip: "bg-red-500/15 text-red-700 dark:text-red-400" },
  skipped: { dot: "bg-zinc-400", labelKey: "portals.statusNoAts", chip: "bg-surface-hover text-muted" },
};
    ...
    <div>
      ...
      {res && !res.available && (
        <p className="mt-4 rounded-xl border border-dashed border-border bg-surface/30 p-4 text-sm text-muted">
          <code className="text-foreground">verify-portals.mjs</code> {t("portals.verifyNotFound")}
        </p>
      )}
      {res && res.available && !res.configured && (
        <p className="mt-4 rounded-xl border border-dashed border-border bg-surface/30 p-4 text-sm text-muted">
          No <code className="text-foreground">portals.yml</code> {t("portals.noPortalsYml")}
        </p>
      )}
      ...
      {res && res.configured && (
        <div className="mt-5">
          <p className="text-sm text-muted">
            <span className="tabular-nums text-emerald-600 dark:text-emerald-400">{liveN}</span> {t("portals.statLive")} ·{" "}
            <span className="tabular-nums text-red-600 dark:text-red-400">{broken.length}</span> {t("portals.statBroken")} ·{" "}
            <span className="tabular-nums">{companies.length}</span> {t("portals.statTracked")}
          </p>
          {broken.length > 0 && (
            <div className="mt-3 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm">
              <span className="font-medium text-red-700 dark:text-red-400">
                {broken.length} {t(broken.length === 1 ? "portals.silentlyDropsSingular" : "portals.silentlyDropsPlural")}
              </span>{" "}
              <span className="text-muted">
                {t("portals.brokenPrefix")} <code className="text-foreground">careers_url</code> {t("portals.brokenMid")}{" "}
                <code className="text-foreground">portals.yml</code> {t("portals.brokenSuffix")}
              </span>
            </div>
          )}
  ...
  if (job?.status === "done")
    return (
      <Link href={`/jobs/${job.id}`} className="text-xs font-medium text-emerald-600 dark:text-emerald-400">
        {t("portals.repairedRecheck")}
      </Link>
    );
  return (
    <button
      onClick={onFix}
      title={t("portals.fixTitle", { company })}
