...
func (t *winTray) wndProc(hWnd windows.Handle, message uint32, wParam, lParam uintptr) (lResult uintptr) {
	...
	case t.wmSystrayMessage:
		log.Printf("systray: callback msg=%d wParam=%d lParam=0x%x", message, wParam, lParam)
		switch lParam {
		case WM_LBUTTONUP:
			systrayLeftClick()
		case WM_RBUTTONUP:
			// SYNCHRONOUS (root-cause fix, Oracle-confirmed): the shell
			// (explorer) delivers this tray notification via SendMessage and
			// expects the context menu to be shown BEFORE this handler returns.
			// The previous design POSTED a wmShowMenu message and returned
			// immediately, showing the menu asynchronously after the SendMessage
			// completed; that left the shell's per-icon "context menu in
			// progress" state stuck, so it suppressed every subsequent
			// right-click (0x205 stopped arriving while 0x200 kept coming) —
			// the menu only appeared once. Running TrackPopupMenu here is the
			// standard Win32 pattern and is safe: wndProc and the window are on
			// the same OS thread (systray.go calls runtime.LockOSThread; the
			// pump runs doNativeTick on that thread), so the modal menu pump
			// owns the window and does NOT corrupt input-queue state. The
			// earlier "hard AV" was caused by the synthetic Alt keypress
			// (keybd_event VK_MENU, abandoned) / a wrong-thread variant, not by
			// a synchronous TrackPopupMenu. See PATCH.md v7.
			if fn := tappedRight; fn != nil {
				fn()
			} else {
				var pt point
				pGetCursorPos.Call(uintptr(unsafe.Pointer(&pt)))
				wt.showMenuAt(int(pt.X), int(pt.Y))
			}
		default:
			log.Printf("systray: callback lParam=0x%x not handled", lParam)
		}
...
// showMenu reads the cursor position and shows the tray context menu there.
// Used by the left-click path and any direct callers; the right-click path
// uses showMenuAt with coordinates captured in wndProc before the menu message
// was posted (see wndProc / wmShowMenu).
func (t *winTray) showMenu() error {
	...
	return t.showMenuAt(int(p.X), int(p.Y))
}

// showMenuAt shows the tray context menu at (x, y). It is called synchronously
// from wndProc while handling the tray right-click callback (WM_RBUTTONUP on
// this app's OWN self-registered tray icon). Because systray.go locks the OS
// thread and the pump runs doNativeTick on that same thread, wndProc and the
// TrackPopupMenu modal loop share one thread — the standard, safe Win32 pattern.
// See PATCH.md v10 for the full history and the getlantern reference.
func (t *winTray) showMenuAt(x, y int) error {
	if !wt.isReady() {
		return ErrTrayNotReadyYet
	}

	log.Printf("systray: showMenu opening at (%d,%d)", x, y)

	// Diagnostics: catch any Go panic inside showMenuAt and write the stack to
	// the tray log. -H windowsgui discards stderr, so an unrecovered panic
	// would kill the process with no trace. This recover keeps the process
	// alive and surfaces the exact crash site.
	defer func() {
		if r := recover(); r != nil {
			buf := make([]byte, 4096)
			n := runtime.Stack(buf, false)
			log.Printf("systray PANIC in showMenuAt: %v\n%s", r, buf[:n])
		}
	}()
	...
	// NOTE (v10): a synthetic Alt keypress (keybd_event, KB Q135788) is NOT used
	// here — it put the system into "menu mode" and swallowed the tray icon's
	// subsequent callbacks. The SPI foreground-lock disable is also NOT used:
	// it was a global anti-pattern that only made SetForegroundWindow success
	// more likely, never reliable, and SPIF_SENDCHANGE disrupted the icon's
	// callback registration. See PATCH.md.
	//
	// FINAL (v11) — the definitive fix is runtime.LockOSThread on the goroutine
	// that runs systray.Run (see tray_windows.go), not further gymnastics in
	// this function. getlantern/systray expects Run to be called on the
	// LockOSThread'd main goroutine, so window creation, message pump, wndProc
	// and TrackPopupMenu all share one pinned OS thread. This app wraps Run in a
	// `go func()`, which left that goroutine unpinned; async preemption could
	// migrate it between shows and corrupt the window/menu/pump thread affinity,
	// which is what made the shell suppress the 0x205 callback after a few
	// shows. With the goroutine pinned, the menu call is the plain, canonical,
	// proven getlantern/systray pattern below: SetForegroundWindow (we are
	// handling the right-click, so our thread is the last input recipient and the
	// foreground lock lets it succeed) + TrackPopupMenu + PostMessage(WM_NULL).
	// No AttachThreadInput (merged input queues with the shell → "false success",
	// v9/v10), no foreground restore (v8 flash-dismiss), no synthetic keypress /
	// SPI hack (v2/v4). See PATCH.md v11.
	sfwRes, _, _ := pSetForegroundWindow.Call(uintptr(t.window))
	log.Printf("systray: SetForegroundWindow res=%d", sfwRes)
	if sfwRes == 0 {
		// The menu still renders — we are the last input recipient; the WM_NULL
		// post below clears the menu-active state best-effort.
		log.Printf("systray: SetForegroundWindow returned 0 (menu should still render)")
	}

	log.Printf("systray: calling TrackPopupMenu")
	...
	log.Printf("systray: TrackPopupMenu returned res=%d", res)
	if res == 0 {
		log.Printf("systray error: TrackPopupMenu failed: %v", err)
		return err
	}

	// CRITICAL RESET: TrackPopupMenu requires the owning thread's "menu active"
	// flag to be cleared before it will display again, and the shell must
	// release its mouse capture for this icon. Windows clears both when the
	// window that owns the menu (t.window) receives a benign WM_NULL message
	// AFTER TrackPopupMenu returns. Without this post, the context menu only
	// appears ONCE — every subsequent right-click fails silently and 0x205
	// stops reaching wndProc. Post to t.window (the owner), never to prevFG.
	// See TrackPopupMenu remarks and PATCH.md v10.
	const WM_NULL = 0x0000
	pPostMessage.Call(uintptr(t.window), WM_NULL, 0, 0)

	log.Printf("systray: showMenu returned ok")
	return nil
}
