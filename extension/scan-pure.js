...
(function () {
  ...
  // BOSS PUA 数字实证映射: PUA \uE031+n → 数字 n,即 E031=0、E032=1 … E039=8、E03A=9。
  //   · E031=0 一段:2026-09-14 用户在 BOSS 页面人工对照 A/B/C 三个独立样本交叉验证,
  //     与 data/pipeline.md 其余样本全量回验一致。
  //   · E03A=9:2026-09-14 四个搜索列表页(约 830 张卡片)取证 —— E032..E039 覆盖数字
  //     1..8,E03A 只在需要 9 的薪资里出现({E039}-{E03A}K / {E037}-{E03A}K /
  //     {E036}-{E03A}K),且 "10" 一律写作两个字形 {E032}{E031}(故 E03A 不可能是 10);
  //     再用两个职位的详情页明文反查确认:8-9K / 6-9K。
  // E030 至今零观测,保持未映射 —— 含未映射码点的薪资清洗后无完整数字 → 归
  // 「薪资未知」,不猜(错数字会错误放行,比「未知」危害大)。若站点轮换字体导致映射
  // 漂移,症状是解码出成片不合常理的薪资,届时用同样的人工对照法重测本表。
  ...
  function toDiscoveredOffer(meta, platform) {
    ...
    // 薪资先解码 PUA 数字、再判未映射残留:混有未映射码点(如至今零观测的 E030)时
    // 部分解码会失真(1?-15K 变 1-15K)→ 整体归「薪资未知」;纯 ASCII/全映射解码
    // 且含数字才携带 salaryText。
