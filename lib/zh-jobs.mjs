...
// (APP, 消息, 张三, company pages) — these patterns keep only the anchors that
// actually point at a posting detail page. zhipin's bare `/job_detail/` root is
// a NAV link (its label is literally 职位搜索), so it must NOT match — only a
// real posting path (`/job_detail/<id>.html`) does.
const ZH_JOB_DETAIL_URL_RE = [
  /zhipin\.com\/job_detail\/[^/?#]+\.html?/i,
  /liepin\.com\/job\/\d+/i,
