...
//     risks 闭集、html 必须在 reports/checkups/ 下**且文件真实存在**（2026-09-17 起；
//     「先写 HTML 再写台账行」——路径合规不等于报告存在，#1022 那行就是这么变成
//     引用空文件的幽灵行的）；
//   - 幂等：整行逐字段完全相同的行已在台账里 → 跳过不追加（重跑 `add` 不再产生第二行）；
//   - 文件缺失时自建并带表头。
//
// 用法：
...
//   node lib/log-checkup.mjs check              # 审计：重复行 / 孤儿 tracker# / 缺 HTML（有发现退 1）
//
...
/** 「无 HTML」哨兵（与 risks 的 `-` 同形不同义，分开命名才读得懂）。 */
const NO_HTML = '-';
...
function parseHtml(input, { exists = existsSync, root = ROOT } = {}) {
  const s = String(input ?? '').trim();
  if (s === '' || s === '-') return '-';
  ...
  // ADR-0030 跟进决议（2026-09-17）：路径合规 ≠ 报告存在。#1022 那行引用了一个从未写出的
  // HTML（2026-09-17 实测），行照样落进台账，而门禁只数行数——两层都不看实体。写入口先设防：
  // 先写 HTML 再写台账行，别让台账先于产物存在。`exists` 是可注入的测试缝（同 buildRow）。
  if (!exists(join(root, ...rel.split('/')))) {
    throw new Error(`非法 html 路径 "${input}"：报告文件不存在（先写 HTML，再写台账行）`);
  }
  return rel;
}
...
export function buildRow(input, opts = {}) {
  const row = [
    ...
    parseHtml(input.html, opts),
    parseNote(input.note),
  ];
  return row.join('\t');
}

/** 整行逐字段相同的行是否已在台账里（append-only 下的幂等判据，2026-09-17 #103 实测：
 *  同一次体检被 `add` 两次，留下两行完全相同的记录，之后再也删不掉）。
 *  只认「完全相同」——同日同 tracker 的复检（note/star 有差异）照常追加。
 *  @param {string} text - 台账原文
 *  @param {string} row - buildRow 的产物
 *  @returns {boolean} */
export function findExactDuplicate(text, row) {
  if (typeof text !== 'string' || !text) return false;
  for (const line of text.split('\n')) {
    if (line.replace(/\r$/, '').trim() === row) return true;
  }
  return false;
}
...
/** 台账审计（纯，`check` 子命令的判据）。append-only 台账有三类自己长不好的伤，
 *  都只能被「发现」而不能被自愈：
 *    - duplicates：整行完全相同（`add` 跑两次；写入口现已幂等拦截，存量行仍会被报出）；
 *    - orphans：tracker# 已不在 tracker 里（行被删/被去重合并）→ 按 tracker# join 的
 *      消费方（web `checkupIndex`）永远显示不出这条体检；
 *    - missing_html：声明了 HTML 而文件不存在（`#1022`，2026-09-17 实测）。
 *  `?` 空键行按 ADR-0025 决议 8 是合法关联键，不算孤儿。
 *  @param {Array<{tracker:string,date:string,slug:string,company:string,star:number,risks:string[],html:string,note:string}>} rows
 *  @param {string} trackerText - data/applications.md 原文（空串 = 读不到 → 跳过孤儿判定）
 *  @param {(rel: string) => boolean} fileExists
 *  @returns {{duplicates: Array, orphans: Array, missing_html: Array}} */
export function auditLedger(rows, trackerText, fileExists = () => true) {
  const numbers = new Set();
  for (const m of String(trackerText ?? '').matchAll(/^\|\s*(\d+)\s*\|/gm)) numbers.add(m[1]);
  const seen = new Set();
  const duplicates = [];
  const orphans = [];
  const missingHtml = [];
  for (const r of rows) {
    const key = [r.tracker, r.date, r.slug, r.company, r.star, r.risks.join(','), r.html, r.note].join('\t');
    if (seen.has(key)) duplicates.push({ tracker: r.tracker, date: r.date, slug: r.slug, company: r.company });
    else seen.add(key);
    if (numbers.size > 0 && r.tracker !== '?' && !numbers.has(r.tracker)) {
      orphans.push({ tracker: r.tracker, date: r.date, company: r.company });
    }
    if (r.html && r.html !== NO_HTML && !fileExists(r.html)) {
      missingHtml.push({ tracker: r.tracker, html: r.html });
    }
  }
  return { duplicates, orphans, missing_html: missingHtml };
}
...
function usage() {
  ...
  console.error('      node lib/log-checkup.mjs check [--pretty] [--tracker-file <data/applications.md>]');
}
...
function cmdAdd(argv) {
  ...
  // 幂等：整行逐字段相同 → 跳过（2026-09-17 #103 实测：重跑 add 给同一次体检留下
  // 第二行完全相同的记录）。不报错：agent 落盘后「顺手再确认一次」是常规动作，
  // 用错误逼它重试只会把一次多余的写入变成一串。退 0 + 显式说明。
  if (findExactDuplicate(existsSync(TSV) ? readFileSync(TSV, 'utf8') : '', row)) {
    console.log(`already logged (idempotent skip): ${row}`);
    return;
  }
...
function cmdCheck(argv) {
  // --pretty 是布尔开关，不走 flagArgs（那里每个 --flag 都必须带值，同 summary）。
  const pretty = argv.includes('--pretty');
  const f = flagArgs(argv.filter((a) => a !== '--pretty'));
  const trackerFile = f['tracker-file'] ?? join(ROOT, 'data', 'applications.md');
  const rows = existsSync(TSV) ? parseLedger(readFileSync(TSV, 'utf8')) : [];
  let trackerText = '';
  try {
    trackerText = readFileSync(trackerFile, 'utf8');
  } catch {
    trackerText = ''; // 读不到 tracker → 跳过孤儿判定（其余两类照报）
  }
  const report = auditLedger(rows, trackerText, (rel) => existsSync(join(ROOT, ...rel.split('/'))));
  console.log(
    JSON.stringify(
      {
        rows: rows.length,
        tracker_file: relative(ROOT, trackerFile).replaceAll('\\', '/'),
        orphans_checked: trackerText !== '',
        ...report,
      },
      null,
      pretty ? 2 : 0,
  ...
  const findings = report.duplicates.length + report.orphans.length + report.missing_html.length;
  if (findings > 0) process.exitCode = 1; // 有发现即非 0，供脚本/定时巡检用
}
